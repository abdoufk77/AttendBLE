...
public class SearchingActivity extends AppCompatActivity {
    ...
    private void handleBeacon(UUID beaconUUID, String code) {
        ...
        // Résout la session active par beaconUUID via le backend pour obtenir le classeId,
        // puis fetch les infos classe. Si l'un des appels échoue, on lance quand même
        // SessionCodeActivity en mode dégradé ("Session detected" générique).
        resolveSessionInfo(beaconUUID, code);
    }

    private void resolveSessionInfo(UUID beaconUUID, String code) {
        ServiceLocator.getSessionRepository().listAllActive(new Callback<List<Session>>() {
            @Override
            public void onSuccess(List<Session> sessions) {
                Session match = null;
                for (Session s : sessions) {
                    if (beaconUUID.toString().equalsIgnoreCase(s.getBeaconUUID())) {
                        match = s;
                        break;
                    }
                }
                if (match == null) {
                    launchSessionCode(beaconUUID, code, null);
                    return;
                }
                fetchClasse(beaconUUID, code, match.getClasseId());
            }
    ...
    private void fetchClasse(UUID beaconUUID, String code, String classeId) {
        ServiceLocator.provideGetClasseUseCase().execute(classeId, new Callback<Classe>() {
            @Override
            public void onSuccess(Classe classe) {
                launchSessionCode(beaconUUID, code, classe);
            }
    ...
    private void launchSessionCode(UUID beaconUUID, String code, Classe classe) {
        ...
        if (classe != null) {
            intent.putExtra(SessionCodeActivity.EXTRA_CLASSE_NAME, classe.getNom());
            StringBuilder meta = new StringBuilder();
            if (classe.getMatiere() != null) meta.append(classe.getMatiere());
            if (classe.getSalle() != null && !classe.getSalle().isEmpty()) {
                if (meta.length() > 0) meta.append(" · ");
                meta.append("Salle ").append(classe.getSalle());
            }
            if (meta.length() > 0) {
